...
import { Carousel, CarouselContent, CarouselItem, CarouselNext, CarouselPrevious } from "@/components/atomics/carousel";
        ...
        <CarouselContent>
          {listings?.data?.data.map((item: Listing, index: number) => (
            <CarouselItem key={index} className="basis-1/4">
              <CardDeals image={item.attachments?.[0] || ""} title={item.title} slug={"/listing/" + item.slug} price={item.price_per_day} wide={item.sqft} capacity={item.max_person} wifi={item.wifi_speed} />
            </CarouselItem>
          ))}
        </CarouselContent>
        <CarouselPrevious />
